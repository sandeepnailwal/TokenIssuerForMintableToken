...
function prepareContract (client){
    ...
    var contractAddress = "0x12B6fd550761EA472e45F7e55840959B7797490F";
...
let totalEthContributed = () =>  {
    ...
    return new Promise(function(resolve,reject)
    {
        contract.methods.totalEthRaised().call(function (err,result) {
...
let transferFrom = (fromAccount, toAddress, amount) =>  {

    let client = prepareClient();

    console.log("Transferring amount : " + amount + " from account : " + fromAccount +" to address : " + toAddress);

    return new Promise(function(resolve,reject)
    {
        client.sendFrom(fromAccount, toAddress, amount, function (err, transactionId) {

            if (err) {
                console.log("Error : " + err);
                reject(err);
            }else {
                console.log("Transfer amount submitted with transaction id : " + transactionId);
...
module.exports = {getBlockNumber,feedReferral,unlockAccount,getBalance,ethContributedBy,totalEthContributed,totalTokensSold};
